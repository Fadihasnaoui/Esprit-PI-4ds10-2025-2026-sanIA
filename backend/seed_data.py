...
COOP_ID     = "11111111-1111-1111-1111-111111111111"
...
ANIMAL_IDS = [str(uuid.uuid4()) for _ in range(8)]
...
def seed():
    ...
    try:
        ...
        # ════════════════════════════════════════
        print("🛠️  Création des tables et de l'hypertable TimescaleDB...")
        Base.metadata.create_all(bind=engine)
        try:
            db.execute("SELECT create_hypertable('animal_telemetry', by_range('time'), if_not_exists => TRUE);")
            db.commit()
        except Exception as e:
            db.rollback()
            print(f"Note: Hypertable creation might have failed or already exists: {e}")
        ...
        user = User(
            id=FARMER_ID, name="احمد الفلاح", email="fellah@sania.ai",
            password_hash=get_password_hash("sania123"),
            ...
            for day_offset in range(7, 0, -1):
                for hour in [6, 10, 14, 18]:
                    ...
                    readings.append(SensorReading(
                        id=str(uuid.uuid4()), field_id=fd["id"],
        ...
        for animal_id in ANIMAL_IDS[:5]:
            ...
            vacc_objects.append(VaccinationLog(
                id=str(uuid.uuid4()), animal_id=animal_id,
        ...
        for atype, sev, status, field_id, note in alerts_data:
            db.add(Alert(
                id=str(uuid.uuid4()), farm_id=FARM_ID,
...
📊 Résumé:
   ...
   👤 1 utilisateur (fellah@sania.ai / sania123)
