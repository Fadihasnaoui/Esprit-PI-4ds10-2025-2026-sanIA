                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                       ...
                       <button onClick={() => {
                          if (confirm("Supprimer ce vaccin réel ?")) {
                             livestockService.deleteVaccination(v.id).then(() => window.location.reload());
                          }
...
const AnimalCard = ({ animal, telemetry, delay, onEdit, onDelete, onPrint, onHealthScan }) => {
  ...
  return (
    <div
      className={`glass-card animal-card ${health.isBlinking ? 'card-status-blink' : ''}`}
      style={{ 
        padding: '1.2rem', 
        display: 'flex', 
        ...
        background: isSVI ? 'rgba(0, 20, 10, 0.4)' : 'var(--panel)',
...
const AnimalsDashboard = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const connectWS = () => {
      ...
      const wsUrl = `${WS_BASE_URL.replace(/^wss?:/, protocol)}/api/v1/livestock_telemetry/ws`;
