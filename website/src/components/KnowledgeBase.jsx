...
import { FileText, Search, ExternalLink, Bug, Activity, AlertTriangle, CheckCircle, Leaf, BookOpen, BarChart3, RefreshCw } from 'lucide-react';
import { diseaseService, alertService, fieldService, ragService } from '../services/api';
...
const KnowledgeBase = () => {
  ...
  const handleRebuild = async () => {
    ...
    try {
      const res = await ragService.rebuildIndex();
      alert(`Indexation terminée : ${res.data.indexed} segments traités.`);
    } catch (err) {
      console.error("Rebuild error:", err);
      alert("Erreur lors de l'indexation. Vérifiez les fichiers dans le dossier Data.");
    } finally {
      ...
      <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <button
          ...
          style={{
            ...
            background: isRebuilding ? 'rgba(139, 195, 74, 0.1)' : 'transparent',
            opacity: isRebuilding ? 0.7 : 1,
            fontSize: '0.8rem',
          }}
        >
          <RefreshCw size={14} className={isRebuilding ? 'animate-spin' : ''} />
          {isRebuilding ? 'Indexation...' : 'Actualiser l\'index IA'}
        </button>
      </div>
          ...
          {activeTab === 'docs' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {filteredDocs.map((doc, i) => (
                <div key={i} className={`glass-card animate-slide-up delay-${i + 1}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.2rem 1.5rem', cursor: 'pointer' }}>
                  ...
                  <button className="btn btn-outline" style={{ padding: '0.45rem 1rem', fontSize: '0.75rem', borderRadius: 'var(--radius-full)' }}>
